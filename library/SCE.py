...
class SCE:
    ...
    def loss_function(self, outputs, labels):
        ...
        outputs = torch.clamp(F.softmax(outputs, dim=1), 1e-7, 1.0)
        label_one_hot = F.one_hot(labels, self.n_classes).float().to(self.device)
        label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0)
        rce = -torch.mean(outputs * torch.log(label_one_hot), dim=1)

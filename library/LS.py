...
class LS:
    ...
    def loss_function(self, outputs, labels, alpha=0.1):
        nllloss = self.criterion(outputs, labels) # original loss
        smoothloss = -torch.mean(F.log_softmax(outputs, dim=1))

        return (1-alpha)*nllloss+alpha*smoothloss
